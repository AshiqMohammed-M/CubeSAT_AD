...
# UI and demo controls use a friendlier alias than the internal catalog.
_ANOMALY_NAME_ALIASES: Dict[str, str] = {
...
class AnomalyInjector:
    ...
    def __init__(
        ...
        self._forced_anomaly_type: Optional[str] = None
    ...
    def maybe_inject(self, telemetry: dict) -> dict:
        ...
        """
        forced_anomaly = self._consume_forced_anomaly()
        if forced_anomaly is not None:
            return self.force_inject(telemetry, forced_anomaly)
    ...
    def force_inject(
        self,
        telemetry_or_type: dict | str,
        anomaly_type: Optional[str] = None,
    ) -> dict | str:
        """
        Force-inject a specific anomaly type.

        When called with ``(telemetry, anomaly_type)``, the anomaly is applied
        immediately. When called with ``(anomaly_type)``, the anomaly is queued
        for the next telemetry frame generated by maybe_inject().

        Raises ValueError for unknown anomaly types.
        """
        if anomaly_type is None:
            queued_type = self._normalise_anomaly_name(str(telemetry_or_type))
            self._validate_anomaly_type(queued_type)
            with self._forced_lock:
                self._forced_anomaly_type = queued_type
            return queued_type

        resolved_type = self._normalise_anomaly_name(anomaly_type)
        self._validate_anomaly_type(resolved_type)

        telemetry = self._apply_perturbation(telemetry_or_type, resolved_type)
        telemetry["anomaly_type"] = resolved_type
    ...
    def _consume_forced_anomaly(self) -> Optional[str]:
        with self._forced_lock:
            forced_anomaly = self._forced_anomaly_type
            self._forced_anomaly_type = None
        return forced_anomaly

    def _normalise_anomaly_name(self, anomaly_type: str) -> str:
        return _ANOMALY_NAME_ALIASES.get(anomaly_type, anomaly_type)

    def _validate_anomaly_type(self, anomaly_type: str) -> None:
